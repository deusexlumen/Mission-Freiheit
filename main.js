...
class Particle {
     ...
     * @param {string} mode - 'election' (Wahl), 'sortition' (Los) oder 'deliberation' (Beratung)
     * @param {Array} centers - Die Gravitationszentren
     */
    update(mode, centers) {
        if (mode === 'election') {
            ...
            if (this.selected) {
                ...
                if (dist > 20) { // Bewegung zum Zentrum
                    this.vx += (dx / dist) * 0.04; // Etwas langsamer als Wahl
                    this.vy += (dy / dist) * 0.04;
                } else {
                    // Im Zentrum angekommen, langsames "Schweben"
                ...
                this.vx += (Math.random() - 0.5) * 0.1; // Weniger Bewegung
                this.vy += (Math.random() - 0.5) * 0.1;
                const speed = Math.sqrt(this.vx*this.vx + this.vy*this.vy);
                if(speed > 1) { // Geschwindigkeitsbegrenzung
                    this.vx = (this.vx/speed);
                    this.vy = (this.vy/speed);
                }
                this.color = '#222'; // Dunkler
                this.size = 0.5;
            }
...
class PhoenixDossier {
    constructor() {
        ...
        this.DOM = {
            ...
            simBtnDeliberate: document.getElementById('btn-deliberate'), // NEU
        ...
        this.simState = {
            ...
            animationFrame: null,
            isRunning: false // Gesteuert durch IntersectionObserver
        };
     ...
     * OPTIMIERT: Nutzt IntersectionObserver, um nur zu zeichnen, wenn sichtbar.
     */
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                ...
                if (isVisible && !audio.paused) {
                    // Wenn Player sichtbar wird UND Audio bereits läuft, Loop starten
                    startVisualizer();
                }
            });
        }, { threshold: 0.1 });

        // Beobachtet die gesamte Audio-Box
    ...
    // --- INTEGRIERTE SIMULATIONS-LOGIK (Optimiert) ---
    
    /**
     * Initialisiert die Canvas-Simulation mit Performance-Optimierung (IntersectionObserver).
     */
    setupSimulation() {
        ...
        this.DOM.simBtnDeliberate.addEventListener('click', () => this.setSimMode('deliberation')); // NEU

        // --- IntersectionObserver für Akku-Schonung ---
        // Die Animation läuft nur, wenn der Canvas tatsächlich im Viewport sichtbar ist.
        this.simState.isRunning = false; // Startet pausiert

        const observerOptions = {
            root: null, // Browser Viewport
            threshold: 0.1 // Startet, sobald 10% sichtbar sind
        };

        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    // Canvas ist sichtbar -> Animation starten (falls noch nicht läuft)
                    if (!this.simState.isRunning) {
                        console.log('Simulation sichtbar: Animation gestartet.');
                        this.simState.isRunning = true;
                        this.loopSimulation();
                    }
                } else {
                    // Canvas nicht mehr sichtbar -> Animation pausieren
                    if (this.simState.isRunning) {
                        console.log('Simulation verlassen: Animation pausiert (Akku sparen).');
                        this.simState.isRunning = false;
                        if (this.simState.animationFrame) {
                            cancelAnimationFrame(this.simState.animationFrame);
                            this.simState.animationFrame = null;
                        }
                    }
                }
            });
    ...
    resizeSimulation() {
        ...
        this.simState.centers = [
            {x: this.simState.width * 0.25, y: this.simState.height * 0.5}, // [0] Wahl-Zentrum 1
            {x: this.simState.width * 0.75, y: this.simState.height * 0.5}, // [1] Wahl-Zentrum 2
            {x: this.simState.width * 0.5, y: this.simState.height * 0.5}  // [2] Deliberations-Zentrum
        ];
     ...
     */
    loopSimulation() {
        // Sicherheitscheck: Stoppt, wenn Kontext fehlt oder Flag auf false steht
        if (!this.simState.ctx || !this.simState.isRunning) {
             if (this.simState.animationFrame) {
                cancelAnimationFrame(this.simState.animationFrame);
                this.simState.animationFrame = null;
            }
        ...
        // NEU: Zeichnet Deliberations-Zentrum (pulsierend)
        ...
        // Fordert den nächsten Frame an (nur wenn isRunning true bleibt)
